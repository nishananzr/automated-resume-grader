...
if groq_client:
    ...
    print("⚠️ WARNING: Groq API key not found. AI Coach feature will be disabled.")
...
class ScoringService:
    ...
    def score_tone(self):
        max_score = 15
        score, feedback = 0, "No experience section found to analyze tone."
        ...
        if self.resume.experience:
            try:
                if len(re.findall(r'(\d+%?|\$\d+[\.,\d]*[kKmMbB]?)', self.resume.experience)) >= 1:
                    score += 10
                if self.get_doc() and sum(1 for t in self.get_doc() if t.pos_ == "VERB" and t.lemma_.lower() in {"managed", "led", "architected", "launched"}) >= 2:
                    score += 10
            except Exception:
                pass
    ...
    def score_role_relevance(self):
        ...
        if target_keywords and self.text:
            ...
            score = int((len(found) / len(target_keywords)) * max_score)
...
async def get_ai_coach_feedback(request: AICoachRequest):
    if not groq_client:
        raise HTTPException(500, "Groq API client not configured.")
