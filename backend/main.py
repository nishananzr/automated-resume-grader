...
spell = SpellChecker()
ROLE_KEYWORDS = {
    "Software Engineer": ["python", "java", "react", "node", "docker", "aws", "sql"],
    "Data Scientist": ["python", "r", "sql", "pandas", "tensorflow"],
    "Designer": ["figma", "sketch", "photoshop", "ui", "ux"]
}
SECTION_HEADERS = {
...
class ScoringService:
    ...
    def get_doc(self):
        if self.doc is None and self.text:
            self.doc = nlp(self.text)
        return self.doc

    def _add_score(self, name, score, max_score, feedback):
        clamped_score = max(0, min(score, max_score))
        self.score_details.append(ScoreDetail(name=name, score=clamped_score, max_score=max_score, feedback=feedback))
        self.total_score += clamped_score

    def calculate_final_score(self):
        current_max = sum(d.max_score for d in self.score_details)
        base_score = 15
        if current_max > 0:
            scaled_score = int((self.total_score / current_max) * (100 - base_score))
            self.total_score = base_score + scaled_score
        else:
            self.total_score = base_score
        return Scorecard(total_score=self.total_score, details=self.score_details)

    def score_all(self):
        self.score_ats_parse_rate()
        self.score_impact_words()
        self.score_clarity_and_brevity()
        self.score_tone()
        if self.role != "General":
            self.score_role_relevance()
    ...
    def score_tone(self):
        ...
        if self.resume.experience:
            try:
                ...
                if pol > 0.15:
                    score, feedback = 15, "Excellent! Positive and confident tone."
                elif pol >= 0.05:
                    score, feedback = 10, "Good, professional tone."
                else:
                    score, feedback = 2, "Tone may seem flat or slightly negative."
            except Exception:
                pass
    ...
    def score_impact_words(self):
        ...
        if self.resume.experience:
            try:
                if len(re.findall(r'(\d+%?|\$\d+[\.,\d]*[kKmMbB]?)', self.resume.experience)) >= 1:
                    score += 10
                doc = self.get_doc()
                if doc and sum(1 for t in doc if t.pos_ == "VERB" and t.lemma_.lower() in {"managed", "led", "architected", "launched"}) >= 2:
                    score += 10
            except Exception:
                pass
    ...
    def score_clarity_and_brevity(self):
        ...
        if self.text:
            ...
            num_errors = len(spell.unknown(words))
            if num_errors == 0:
                score += 15
            elif num_errors <= 3:
                score += 10
...
# --- Helper function to manually read the secret file ---
def get_secret_from_file(key_name: str) -> Optional[str]:
    env_path = '.env'
...
async def rescore_resume(request: ReScoreRequest):
    ...
    scorer = ScoringService(parsed_data, role=request.role)
    return scorer.score_all()

@app.post("/tailor-resume/")
async def tailor_resume(request: TailoringRequest):
    jd_doc = nlp(request.job_description)
    resume_doc = nlp(request.resume_text)
...
async def get_ai_coach_feedback(request: AICoachRequest):
    ...
    if not api_key:
        api_key = get_secret_from_file("GROQ_API_KEY")

    if not api_key:
        print("❌ CRITICAL: Could not read GROQ_API_KEY from environment or .env file.")
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail="The AI Coach service is experiencing issues.")
